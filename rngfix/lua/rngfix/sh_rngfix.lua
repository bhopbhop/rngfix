...
local g_cvStairs = CreateConVar("rngfix_stairs", "0", CV_FLAGS, "Enable stair slide fix (surf only)", 0.0, 1.0)
...
local function DoTelehopFix(ply, mv)
	...
	-- BUG: it increases player velocity
...
local function DoStairsFix(ply, mv)
	...
	Debug.Msg(ply, "DO FIX: Stair Sliding")
	Debug.Laser(ply, ply:GetPos(), endPos, 2)
